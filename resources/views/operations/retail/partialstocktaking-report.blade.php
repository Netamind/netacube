...
<title>Partial Stocktaking Report – {{ $branchName }} – {{ $displayDate }}</title>
<style>
...
<div class="container">
    @php
        ...
        $countedRows    = $countedRows ?? collect();
        ...
        // rectified yet (Download Report is available either way).
        ...
        // padded with X if short) + PSR (Partial Stocktaking Report) + date.
        $branchPrefix = strtoupper(substr(preg_replace('/[^A-Za-z]/', '', $branchName ?: 'BR'), 0, 3));
        $branchPrefix = str_pad($branchPrefix, 3, 'X');
        $docNumber    = $branchPrefix . 'PSR' . \Carbon\Carbon::parse($date ?? $displayDate)->format('Ymd');
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="doc-details">
                <p><strong>Partial Stocktaking Report</strong></p>
    ...
    @if($remarks)
    <!-- REMARKS -->
    <div class="section-title">Auditor Remarks</div>
    ...
    <table class="address-table">
        <tr>
            ...
            <td>
                ...
                <p>Difference (FV − EV): <strong style="color:#dc2626;">MWK {{ number_format($difference, 2) }}</strong></p>
            </td>
        </tr>
    </table>
    ...
    <div class="footer">
        ...
        <p>{{ $branchName }} &middot; Partial Stocktaking Report &middot; {{ $displayDate }} &middot; Generated {{ now()->format('d M Y, H:i') }}</p>
    </div>
