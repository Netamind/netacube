@extends('operations.dashboard')

{{--
    resources/views/operations/dashboard-settings.blade.php

    Settings page for OperationsDashboardSettingsController. Extends
    operations/dashboard.blade.php (the Operations landing-hub layout).
    Sidebar link already lives in that layout under System > Settings
    (route: tenant.operations.dashboard.settings).
    ...
    Note: operations/dashboard.blade.php already @yields('styles') in its
    <head> and wraps @yield('content') in .content-page > .content >
    .container-fluid itself, so — unlike the Admin equivalent — this page
    doesn't need to add that wrapper or a separate @section('styles')
    block; styles are still kept inline within @section('content') to
    match the same format tenants/admin/dashboard-settings-admin.blade.php
    uses.
