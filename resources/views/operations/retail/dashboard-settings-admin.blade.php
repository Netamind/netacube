...
{{--
    resources/views/operations/retail/dashboard-settings-admin.blade.php

    Admin's Dashboard Settings, rendered under operations/retail/dashboard.blade.php
    (the Retail layout/sidebar) instead of tenants/admin/dashboard.blade.php.
    Reached only when an Admin clicks Settings > Dashboard from inside Retail
    — see OperationsDashboardSettingsController::showRetailSettingsView,
    which detects Admin and hands this view the same data
    AdminDashboardSettingsController::gatherSettingsData() produces for the
    Admin's own sidebar page (tenants.admin.dashboard-settings-admin).

    tenant.admin.dashboard and tenant.operations.dashboard (the hub) are
    untouched by this — this view exists purely so an Admin browsing Retail
    doesn't get bounced into the Admin theme when they open Settings; they
    stay inside Retail's chrome the whole time. Content/fields are otherwise
    identical to tenants.admin.dashboard-settings-admin.blade.php — same
    admin_dashboard_settings table, same validation, same
    AdminDashboardSettingsController::updateSettings() on submit.

    Note: operations/retail/dashboard.blade.php does NOT auto-wrap
    @yield('content') and has no @yield('styles') (same pattern as
    tenants/admin/dashboard.blade.php) — so, same as the Admin original,
    content here is manually wrapped in .content-page > .content >
    .container-fluid, and styles stay inline within @section('content').
