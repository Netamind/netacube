...
{
    public function showMasterLoginPage() { return view('master.login'); }
    public function showMasterDashboard() { return view('master.dashboard'); }
  
   
    public function showProfileView() { return view('master.profile'); }
    public function showEmployeesView() { return view('master.employees'); }
    public function showEmployeesDetailsView() { return view('master.employee-details'); }
    public function showCompanyInfoView() { return view('master.company-info'); }
    public function showSubscriptionPlansView() { return view('master.subscription-plans'); }
    public function showPaymentMethodsView() { return view('master.payment-methods'); }
    public function showRolesView() { return view('master.roles'); }
    public function showMasterSupportCenter() { return view('master.support-center'); }
    public function showMasterEvents() { return view('master.events'); }
    ...
    public function updateCompanyGeneralInfo(Request $request)
    {
        ...
        if ($validator) {
            $updated = DB::table('company_info')->where('id', 1)->update($data);
            if ($updated) {
...
public function insertSubscriptionPlan(Request $request)
{
    ...
    if ($validator) {
        $insertId = DB::table('subscription_plans')->insertGetId($data);
        if ($insertId) {
            $plan = DB::table('subscription_plans')->where('id', $insertId)->first();
...
public function updateSubscriptionPlan(Request $request)
{
    ...
    if ($validator) {
        $updated = DB::table('subscription_plans')->where('id', $request->id)->update($data);
        if ($updated) {
            $plan = DB::table('subscription_plans')->where('id', $request->id)->first();
...
{
    $deleted = DB::table('subscription_plans')->where('id', $request->id)->delete();
    ...
    {
        return view('master.events-table');
    }

    public function fetchMasterEvents(Request $request)
    {
    ...
    public function storeMasterEvent(Request $request)
    {
        ...
        if ($validator) {
            ...
            if ($insertId) {
                $event = DB::table('events')->where('id', $insertId)->first();
                $start = $event->start_date . ($event->all_day ? '' : 'T' . $event->start_time);
                $end = $event->all_day
                    ? Carbon::parse($event->end_date)->addDay()->format('Y-m-d')
                    : ($event->end_date . 'T' . $event->end_time);
    ...
    public function updateMasterEvent(Request $request, $id)
    {
        $startTime = $request->all_day ? null : ($request->start_time ?? null);
        $endTime = $request->all_day ? null : ($request->end_time ?? null);
        ...
        if ($validator) {
            $updated = DB::table('events')->where('id', $id)->where('user_id', Auth::user()->id)->update($data);
            if ($updated) {
    ...
    public function destroyMasterEvent($id)
    {
        $deleted = DB::table('events')->where('id', $id)->where('user_id', Auth::user()->id)->delete();
        if ($deleted) {
    ...
    public function bulkDeleteMasterEvents(Request $request)
    {
        $validator = $request->validate([
            'ids' => 'required|array',
    ...
    public function addEventViewTable(Request $request)
    {
        $startTime = $request->all_day ? null : ($request->start_time ?? null);
        $endTime = $request->all_day ? null : ($request->end_time ?? null);
...
public function uploadDocument(Request $request)
   {
    ...
    public function insertEmployee(Request $request)
    {
        ...
        $id = DB::table('users')->insertGetId([
            ...
            'password' => Hash::make('default123'), // temp password
            'active' => 'Yes',
            'entered_on' => now(),
        ]);
...
public function insertPaymentMethod(Request $request)
{
    ...
    if ($validator) {
        $insertId = DB::table('payment_methods')->insertGetId($data);
        if ($insertId) {
            $method = DB::table('payment_methods')->where('id', $insertId)->first();
...
{
    $current = DB::table('payment_methods')->where('id', $request->id)->first();
    ...
    if ($validator) {
        $updated = DB::table('payment_methods')->where('id', $request->id)->update($data);
        if ($updated || $updated === 0) {
            $method = DB::table('payment_methods')->where('id', $request->id)->first();
...
{
    $deleted = DB::table('payment_methods')->where('id', $request->id)->delete();
...
{
    return view('master.invoice-templates');
}
...
public function insertInvoiceTemplate(Request $request)
{
    ...
    if ($validator) {
        ...
        $insertId = DB::table('invoice_templates')->insertGetId($data);
        if ($insertId) {
            $template = DB::table('invoice_templates')->where('id', $insertId)->first();

            if ($data['is_default']) {
                DB::table('invoice_templates')->where('id', '!=', $insertId)->update(['is_default' => 0]);
            }
...
public function updateInvoiceTemplate(Request $request)
{
    ...
    if ($validator) {
        $old = DB::table('invoice_templates')->where('id', $request->id)->first();
        $path = base_path('resources/views/common/invoices');
        ...
        if ($data['is_default']) {
            DB::table('invoice_templates')->where('id', '!=', $request->id)->update(['is_default' => 0]);
        }
        ...
        $updated = DB::table('invoice_templates')->where('id', $request->id)->update($data);
...
public function deleteInvoiceTemplate(Request $request)
{
    ...
    $deleted = DB::table('invoice_templates')->where('id', $request->id)->delete();
...
public function previewInvoiceTemplate($filename)
{
    ...
    $cleanName = pathinfo($filename, PATHINFO_FILENAME); // e.g., "malawi_pro.blade"
    $cleanName = preg_replace('/\.blade$/i', '', $cleanName); // → "malawi_pro"
...
public function previewInvoiceTemplatePdf($filename)
    {
        ...
        // Optional: Pass data to the view (e.g. sample invoice data)
        $data = [
        ...
        // Stream PDF directly (so it shows in browser)
    ...
    public function insertCurrency(Request $request)
{
    ...
    if ($validator) {
        $insertId = DB::table('currency')->insertGetId($data);
        if ($insertId) {
            $currency = DB::table('currency')->where('id', $insertId)->first();
...
public function updateCurrency(Request $request)
{
    ...
    if ($validator) {
        $updated = DB::table('currency')->where('id', $request->id)->update($data);
        if ($updated) {
            $currency = DB::table('currency')->where('id', $request->id)->first();
...
{
    $deleted = DB::table('currency')->where('id', $request->id)->delete();
