...
{{--
    resources/views/tenants/admin/dashboard-settings-admin.blade.php

    Settings page for AdminDashboardSettingsController. Extends
    tenants.admin.dashboard (the Admin layout/sidebar). Sidebar link already
    lives in that layout under System > Dashboard Settings
    (route: tenant.admin.system.dashboard.settings).

    admin_dashboard_settings is now per-user, same shape as
    operations_dashboard_settings / sales_dashboard_settings — every Admin
    gets their own row, created lazily on first save. $hasSavedSettings
    (from AdminDashboardSettingsController::showSettingsView) drives the
    "not saved yet" notice/modal below, same pattern Operations uses.

    Note: tenants.admin.dashboard only @yields('content') and @yields('scripts')
    — no separate styles section — so styles are inline within @section('content')
    rather than a @section('styles') block.

    Page is wrapped in .content-page > .content > .container-fluid, matching
    the outer boundary every other Admin page (e.g. Salary Advances) uses —
    without it the card renders wider than the template's content area.

    Visible fields are trimmed to match operations/dashboard-settings.blade.php's
    set for now (Default Landing Sector, Sector Switching, Idle Timeout,
    Session Lifetime) — the remaining Admin-only fields (Single Session,
    Dashboard Behaviour, Alerts) still validate and save via hidden inputs
    carrying their current/default value, they just don't have visible
    controls yet. Add their fields back in when that UI is built.
...
<form action="{{ route('tenant.admin.system.dashboard.settings.admin.update', ['tenantName' => $tenantName ?? request()->route('tenantName')]) }}" method="POST" id="adminSettingsForm" autocomplete="off">
@csrf
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4 class="mb-0"><i class="ri-shield-user-line"></i> Admin Dashboard Settings</h4>
    ...
    <div class="card-body">
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select" name="default_landing_sector" data-default="">
                    <option value="">— No default (land on the Admin dashboard) —</option>
                    @foreach($availableSectors as $sector)
                        <option value="{{ $sector }}" {{ old('default_landing_sector', optional($adminSettings)->default_landing_sector) === $sector ? 'selected' : '' }}>
                            {{ $sector }}
                        </option>
                    @endforeach
                </select>
                <div class="settings-help">If set, Admins land directly on this sector's dashboard after login instead of the general Admin dashboard.</div>
            </div>
            <div class="col-md-6 mb-3">
                ...
                <div class="form-check form-switch">
                    ...
                    <input class="form-check-input" type="checkbox" role="switch" name="allow_sector_switching" value="1"
                           id="allow_sector_switching" data-default-checked="1" {{ old('allow_sector_switching', optional($adminSettings)->allow_sector_switching) ? 'checked' : '' }}>
            ...
            <div class="col-md-3 mb-3">
                ...
                <div class="form-check form-switch">
                    ...
                    <input class="form-check-input" type="checkbox" role="switch" name="idle_timeout_enabled" value="1"
                           id="idle_timeout_enabled" data-default-checked="1" {{ old('idle_timeout_enabled', optional($adminSettings)->idle_timeout_enabled) ? 'checked' : '' }}>
                    <label class="form-check-label" for="idle_timeout_enabled">Enabled</label>
                </div>
            </div>
            <div class="col-md-3 mb-3">
                <label class="form-label">Idle Timeout (minutes)</label>
                <input type="number" class="form-control" name="idle_timeout_minutes" min="1" max="1440" step="1" data-default="60" autocomplete="off"
                       value="{{ old('idle_timeout_minutes', optional($adminSettings)->idle_timeout_minutes ?? 60) }}" required>
            </div>
            <div class="col-md-3 mb-3">
                <label class="form-label">Session Lifetime (minutes)</label>
                <input type="number" class="form-control @error('session_lifetime_minutes') is-invalid @enderror" name="session_lifetime_minutes" min="1" max="10080" step="1" data-default="120" autocomplete="off"
                       value="{{ old('session_lifetime_minutes', optional($adminSettings)->session_lifetime_minutes ?? 120) }}" required>
        ...
        {{-- Admin-only fields not shown yet (match Operations' visible set for now) —
             carried as hidden inputs so they keep their current value on save. --}}
        <input type="hidden" name="enforce_single_session" data-default="0"
               value="{{ old('enforce_single_session', optional($adminSettings)->enforce_single_session) ? '1' : '0' }}">
        <input type="hidden" name="dashboard_refresh_interval_seconds" data-default="300"
               value="{{ old('dashboard_refresh_interval_seconds', optional($adminSettings)->dashboard_refresh_interval_seconds ?? 300) }}">
        <input type="hidden" name="default_report_range" data-default="today"
               value="{{ old('default_report_range', optional($adminSettings)->default_report_range ?? 'today') }}">
        <input type="hidden" name="low_stock_alert_enabled" data-default="1"
               value="{{ old('low_stock_alert_enabled', optional($adminSettings)->low_stock_alert_enabled) ? '1' : '0' }}">
        <input type="hidden" name="low_stock_alert_threshold" data-default="10"
               value="{{ old('low_stock_alert_threshold', optional($adminSettings)->low_stock_alert_threshold ?? 10) }}">
        <input type="hidden" name="email_notifications_enabled" data-default="1"
               value="{{ old('email_notifications_enabled', optional($adminSettings)->email_notifications_enabled) ? '1' : '0' }}">
        <input type="hidden" name="notification_email" data-default=""
               value="{{ old('notification_email', optional($adminSettings)->notification_email) }}">
...
{{-- ══ CONFIRM DEFAULTS MODAL ══
     For an Admin with no saved row: submits the form as-is via the
     form="adminSettingsForm" attribute, independent of Reset Defaults —
...
<div class="modal fade" id="settingsInfoModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog"><div class="modal-content" style="border:none;border-radius:10px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.18);">
    ...
    <div class="modal-body" style="padding:18px 20px !important;font-size:13.5px;color:#334155;">
      <p>These settings are personal to your account — they don't affect any other Admin on this tenant.</p>
      <p class="mb-0"><strong>Default Landing Sector</strong> only affects what happens right after login. If it's set, you'll skip straight past this dashboard. It has no effect once already logged in.</p>
    </div>
    <div class="modal-footer" style="padding:10px 20px 14px;">
...
{{-- Server-flashed result of the last form submission (Save Changes or
     ...
     AdminDashboardSettingsController::updateSettings). --}}
@if(session('message'))
<script>
