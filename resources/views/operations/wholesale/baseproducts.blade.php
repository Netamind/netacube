...
$(document).ready(function () {
    ...
    function bpUploadRowsChunked(validRows, supplierId, clientBatchId) {
        ...
        function uploadOne(index) {
            ...
            return new Promise(function(resolve) {
                $.ajax({
                    type:        'POST',
                    url:         '{{ route("wholesale.operations.baseproducts.csv.upload") }}',
    ...
    $('#submitSingleDeleteBtn').on('click', function(e) {
        ...
        $.ajax({
            type:    'POST',
            url:     '{{ route("wholesale.operations.baseproducts.delete") }}',
            timeout: 60000,
    ...
    $('#submitMultipleDeleteBtn').on('click', function(e) {
        ...
        $.ajax({
            type:    'POST',
            url:     '{{ route("wholesale.operations.baseproducts.bulkdelete") }}',
            timeout: 60000,
    ...
    function doBulkStatus(isProduct) {
        ...
        $.ajaxSetup({ headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')} });
        $.ajax({
            type: 'POST', url: '{{ route("wholesale.operations.baseproducts.bulkstatus") }}',
    ...
    $('#applyBulkSupplierBtn').on('click', function(e) {
        ...
        $.ajaxSetup({ headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')} });
        $.ajax({
            type: 'POST', url: '{{ route("wholesale.operations.baseproducts.bulksupplier") }}',
            data: { ids:selected, supplier_id:supplier, _token:'{{ csrf_token() }}' }, timeout:60000,
            beforeSend: function() { $('#progressBar').show(); },
            complete:   function() { $('#progressBar').hide(); },
            success: function(data) {
                if (data.status === 201) {
