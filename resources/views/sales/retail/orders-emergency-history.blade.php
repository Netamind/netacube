...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    ...
    <div class="d-flex align-items-center" style="gap:4px;">
      ...
      <a href="{{ route('retail.orders.emergency') }}" class="btn btn-light text-primary fs-16 mx-1" title="New order">
    ...
    <ul class="nav nav-pills px-2 pt-1">
      <li class="nav-item">
        <a class="nav-link" href="{{ route('retail.orders.emergency') }}"><i class="ri-alarm-warning-line"></i> Emergency</a>
      </li>
      <li class="nav-item">
        <a class="nav-link active" href="{{ route('retail.orders.emergency.history') }}"><i class="ri-history-line"></i> History</a>
      </li>
    </ul>
  </div>
...
const ROUTES = {
    ...
    statusUpdate: '{{ route("retail.orders.status.update") }}',
    lineUpdate: '{{ route("retail.orders.line.update") }}',
    lineDelete: '{{ route("retail.orders.line.delete") }}',
    bulkDelete: '{{ route("retail.orders.bulkdelete") }}',
...
// ─────────────────────────────────────────────────────────────────────────
$(document).on('change', '.rowCheck, #selectAllRows', function () {
    if (this.id === 'selectAllRows') {
        $('.rowCheck').prop('checked', this.checked);
    }
    const anyChecked = $('.rowCheck:checked').length > 0;
...
$('#bulkDeleteBtn').on('click', function (e) {
    ...
    openDeleteConfirm(ids.length + ' selected item(s)', function () {
        $.post(ROUTES.bulkDelete, { ids }).done(function () {
            toastr.success('Selected items deleted.');
            $('#deleteOrderModal').modal('hide');
            location.reload();
        }).fail(function () {
            toastr.error('Could not delete the selected items.');
            $('#deleteOrderModal').modal('hide');
        });
    });
});
...
// ─────────────────────────────────────────────────────────────────────────
// STATUS CHANGE — inline per-row select
// ─────────────────────────────────────────────────────────────────────────
$(document).on('change', '.row-status-select', function () {
    const id = $(this).data('id');
    const status = $(this).val();
    $.post(ROUTES.statusUpdate, { id, status }).done(function () {
