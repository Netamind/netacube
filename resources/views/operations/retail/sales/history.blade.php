...
@php
    ...
    // "today" is kept as the system default AND as the default target date
    // for the Change Date modal — $date is the one actually being viewed.
    $today       = Carbon::today()->toDateString();
    $customDate  = $pref->date ?? null;
    $isCustom    = !empty($customDate);
    $date        = $isCustom ? $customDate : $today;
    ...
    $tableTitle = $branchName . ' Sales History [' . $displayDate . ']';
@endphp
...
.header-title-block { display:flex; flex-direction:row; align-items:center; gap:10px; line-height:1.25; min-width:0; }
.header-date-line { font-size:12px; font-weight:400; opacity:.85; margin-top:0; display:flex; align-items:center; gap:4px; white-space:nowrap; }

/* ── Date chip (select history date) — same pattern as Action Center ─ */
.header-date-line.ch-date-chip {
...
.header-date-line.ch-date-chip:hover { background:rgba(255,255,255,0.28); }
.header-date-line.ch-date-chip .mode-badge {
    font-size:9px; padding:1px 5px; border-radius:8px;
    background:rgba(255,255,255,0.2); color:#fff; font-weight:600; letter-spacing:.3px;
}
.header-date-line.ch-date-chip.custom-mode { background:rgba(245,158,11,0.30); border-color:rgba(245,158,11,0.6); }
.header-date-line.ch-date-chip.custom-mode .mode-badge { background:rgba(245,158,11,0.5); }
.header-date-line.ch-date-chip .chip-edit-icon { font-size:10px; opacity:.75; margin-left:2px; }
.header-date-line.ch-date-chip:hover .chip-edit-icon { opacity:1; }
...
.dmc-desc { font-size:11px; color:#94a3b8; margin-top:1px; }
...
@media (max-width: 576px) {
  ...
  .header-title-block { max-width:55vw; flex-wrap:wrap; row-gap:4px; }
    ...
    <h4 class="header-title mb-0">
      ...
      <form method="POST" action="{{ route('retail.operations.update.filters') }}"
        ...
        <div class="header-title-block">
          ...
          @if($selectedBranch)
          <span class="header-date-line ch-date-chip {{ $isCustom ? 'custom-mode' : '' }}" id="dateChip" title="Change date">
            <i class="ri-calendar-line"></i> <span id="dateChipText">{{ $displayDate }}</span>
  ...
  <div class="card-body">
    @if(!$selectedBranch)
      <div class="no-branch-wrap">
        ...
        <p style="font-size:13px;">Select a branch from the header above to view sales history.</p>
      </div>
    @else
...
<div class="modal fade" id="dateModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1">
  <div class="modal-dialog" style="max-width:400px;">
    <div class="modal-content" style="border:none;border-radius:10px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.18);">
      <div class="modal-header mh-blue">
        <div class="mh-title-block">
          <div style="font-size:15px;font-weight:700;color:#fff;">Sales Date</div>
        </div>
        <button type="button" class="btn-close mh-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body" style="padding:20px;">
        <div class="date-mode-toggle">
          <div class="dmc {{ !$isCustom ? 'active-sys' : '' }}" id="dmcSystem" onclick="setDateMode('system')">
            <div class="dmc-label">Today</div>
            <div class="dmc-val">{{ Carbon::today()->format('d M Y') }}</div>
            <div class="dmc-desc">Auto-updates daily</div>
          </div>
          <div class="dmc {{ $isCustom ? 'active-cus' : '' }}" id="dmcCustom" onclick="setDateMode('custom')">
            ...
            <div class="dmc-desc">Any past date</div>
          </div>
        </div>
        <form method="POST" action="{{ route('retail.operations.update.filters') }}" id="dateForm">
          @csrf
          <input type="hidden" name="user_id" value="{{ Auth::id() }}">
          <input type="hidden" name="date" id="dateFormValue" value="">
          <div id="customDateRow" style="{{ $isCustom ? '' : 'display:none;' }}">
            <label style="font-size:12px;font-weight:600;color:#374151;">Select date</label>
            <input type="date" class="form-control mt-1" id="customDateInput"
                   max="{{ $today }}"
                   value="{{ $isCustom ? $date : $today }}"
      ...
      <div class="modal-body" style="padding:18px 20px;">
        ...
        <div id="details-intervals-pane">
          @if($detailsRows->isEmpty())
            <p style="text-align:center;color:#94a3b8;padding:20px 0;">No interval sales recorded for the selected date.</p>
          @else
        ...
        <div style="max-height:55vh;overflow-y:auto;">
          <table class="plain-tbl">
            ...
            <tbody id="ivTbody">
              ...
              @empty
                <tr id="ivEmptyRow">
                  <td colspan="3" style="text-align:center;color:#94a3b8;padding:24px;border:none;">No interval sales recorded for the selected date.</td>
                </tr>
              @endforelse
...
$(document).ready(function () {
    ...
    /* ── Date modal (System/Custom toggle — same pattern as Action Center) */
    ...
    function recalcIvGrandTotal() {
        ...
        if (remaining === 0) {
            ...
            if (!$('#ivEmptyRow').length) {
                $('#ivTbody').append('<tr id="ivEmptyRow"><td colspan="3" style="text-align:center;color:#94a3b8;padding:24px;border:none;">No interval sales recorded for the selected date.</td></tr>');
            }
        } else {
            $('#ivEmptyRow').remove();
